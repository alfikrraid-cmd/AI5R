...
from MEMORY.memory_pipeline import EnterpriseMemoryPipeline


def create_learning():

    reality = {
        "object_id": "reality-401",
        "object_type": "reality",
        "asset": "pump-401",
        "measurements": {
            "vibration": 15,
    ...
    return LearningEngine().process(outcome)
...
def test_memory_pipeline():
    ...
    pipeline = EnterpriseMemoryPipeline()

    result = pipeline.run(learning)

    assert result["registry_size"] == 1

    assert result["memory"].learning_id == learning.learning_id

    assert result["memory"].status == "memorized"


def test_pipeline_returns_learning():
    ...
    assert result["learning"].learning_id == learning.learning_id
