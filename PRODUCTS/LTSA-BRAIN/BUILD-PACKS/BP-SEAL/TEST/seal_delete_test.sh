#!/usr/bin/env bash
# Functional test for WF-LTSA-BRAIN-SEAL-DELETE-001 (Seal Delete).
...
# with seal_detail_test.sh / seal_update_test.sh fixtures).
set -euo pipefail
...
psql_run -c "INSERT INTO seal_registry (seal_code, seal_name) VALUES ('${TEST_CODE}', 'TEST SEAL DELETE');"

echo "[1/2] Existing record is removed; a subsequent lookup confirms removal"
psql_run -c "DELETE FROM seal_registry WHERE seal_code = '${TEST_CODE}';"
REMAINING=$(psql_run -tAc "SELECT count(*) FROM seal_registry WHERE seal_code = '${TEST_CODE}';")
...
DELETED=$(psql_run -tAc "DELETE FROM seal_registry WHERE seal_code = 'DOES-NOT-EXIST-$$' RETURNING seal_code;")
if [ -n "${DELETED}" ]; then
...
echo "ALL DB-LEVEL CHECKS COMPLETE for WF-LTSA-BRAIN-SEAL-DELETE-001"
