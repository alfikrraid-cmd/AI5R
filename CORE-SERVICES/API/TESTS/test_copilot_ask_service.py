...
def _ask(question, *, installation_records=(), cmon_records=(), stock_pools=(), seal_leak_diagnostic_service=None, tag=None, ltsa_knowledge_service=None):
    return ask_copilot(
        question,
        tag,
        None,
...
# --- seal leak diagnostic integration ----------------------------------------
...
class _DiagnosticKnowledgeService:
    ...
    def build(self, tag):
        ...
        reading_date = (date.today() - timedelta(days=2)).isoformat()
        readings = []
        if self.leak:
            readings = [
                {
