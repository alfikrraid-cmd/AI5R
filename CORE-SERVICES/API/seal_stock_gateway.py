...
LIST_PATH = "ltsa/seal-stock/list"


@dataclass(slots=True)
class SealStockGatewayConfig:
...
class SealStockGateway:
    ...
    Seal Stock n8n workflow (BUILD-PACKS/BP-SEAL-STOCK, MWO-LTSA-030) and
    ...
    (MWO-INV-CTX-001) needs to resolve a spare part's availability,
    reorder point, and location; the existing create/detail/update/delete
    n8n workflows are left unexposed here until a real Inventory Module
    MWO requests them.
    """

    def __init__(self, config: SealStockGatewayConfig | None = None):
        self.config = config or SealStockGatewayConfig(
            base_url=os.getenv("AI5R_SEAL_STOCK_GATEWAY_BASE_URL", DEFAULT_BASE_URL),
        )

    def list_seal_stocks(self) -> dict[str, Any]:
