...
UI-014–UI-018, then UI-015–UI-019, then UI-016–UI-021, then UI-017–UI-023;
renumbered again to UI-018–UI-025 to avoid colliding with the now-completed
UI-017 (Command Toolbar Framework v1).

- UI-018 Workspace ordering/pinning — persist explicit tab order instead of
  registration/open order.
- UI-019 Multi-consumer workspace support — generalize the
...
- UI-020 Workspace storage versioning/migration — guard against stale
  `localStorage` shapes if the persisted schema changes again.
- UI-021 Clarify/remove the inert app-level `WorkspaceProvider` in
  `app/providers.jsx` (see `TECHNICAL_DEBT.md`, found in UI-012).
- UI-022 Migrate `modules/ltsa/components/Panel.jsx` to the new design-system
  `Panel` (from UI-013), once a module is ready to adopt it.
- UI-023 Docking drag-drop + persistence — generalize `DockManager` (from UI-014)
  with panel↔area reassignment and a storage layer, once a consumer needs it.
- UI-024 Inspector field editing — add `onChange`/controlled editing to
  `InspectorField` (from UI-015), once a consumer needs mutation, not just display.
- UI-025 DataGrid server-side sort/page + column resize/reorder — generalize
  `DataGrid` (from UI-016) once a consumer needs large/remote datasets.
- UI-026 Toolbar dropdown/menu buttons + keyboard-shortcut display — generalize
