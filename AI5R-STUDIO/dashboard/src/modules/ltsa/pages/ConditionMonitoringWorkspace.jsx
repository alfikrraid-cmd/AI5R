...
export default function ConditionMonitoringWorkspace({ navContext, onNavigate }) {
  ...
  // LTSA_CM_UI_REMEDIATION_R1E -- the newest reading among the LOADED page of
  // readings for this pump (history ordering), shown and labelled as such.
  // It is not the canonical Current Condition (cm_condition_evaluator via the
  // Knowledge summary); this workspace makes no Current Condition claim.
  ...
  return <WorkspaceShell className="cmon-workspace-root" theme={theme}>
    ...
    <div className="workspace-shell"><AssetSelector assets={pumps} selectedTag={selectedTag} onSelect={setSelectedTag} />
      {loading ? <div className="workspace-empty">Loading condition monitoring data...</div> : readingsError ? <div className="workspace-empty" role="alert">{readingsError}</div> : !selectedTag ? <div className="workspace-empty">Select a pump to observe its condition.</div> : <>
        <section className="cmon-identity"><span className="eyebrow">Observation workspace</span><h1>{pump?.name ?? selectedTag}</h1><p>{pump?.area ?? "Area unavailable"} - latest loaded capture: {latest?.readingDate ?? "Unavailable"}</p></section>
        <section className="cmon-health-grid"><article className="cmon-health-card"><span className="eyebrow">Overall health</span><strong>Unavailable</strong><span>Health score service is not available.</span></article><article className="cmon-health-card" data-testid="latest-loaded-reading-card"><span className="eyebrow">Latest loaded reading</span><strong>{latest?.readingDate ?? "Unavailable"}</strong><span>{latest ? "Newest reading on this page, not the canonical Current Condition" : "No reading loaded"}</span></article><article className="cmon-health-card"><span className="eyebrow">Monitoring status</span><strong>{schedule?.frequency ?? "Unavailable"}</strong><span>{schedule ? "Schedule available" : "No schedule available"}</span></article><article className="cmon-health-card"><span className="eyebrow">Intervention</span><strong>Unavailable</strong><span>Recommendation service is not available.</span></article></section>
        <div className="workspace-grid"><main className="main-column"><section className="assessment-section"><div className="section-head"><div><span className="eyebrow">Measurements</span><h2>Latest loaded reading</h2>{latest ? <span className="confidence-label">Reading {latest.id} · {latest.readingDate ?? "date unavailable"}</span> : null}</div><span className="status-signal neutral">Read-only evidence</span></div>{latest ? <div className="cmon-reading-grid">{readingItems(latest).map(([label, value, suffix]) => <div className="cmon-reading" key={label}><span>{label}</span><b className="tabular">{fmt(value, suffix)}</b></div>)}<div className="cmon-reading"><span>Vibration</span><b>Unavailable</b></div><div className="cmon-reading"><span>Pressure</span><b>Unavailable</b></div><div className="cmon-reading"><span>Flow</span><b>Unavailable</b></div><div className="cmon-reading"><span>Oil condition</span><b>Unavailable</b></div><div className="cmon-reading"><span>Bearing condition</span><b>Unavailable</b></div></div> : unavailable("Latest loaded reading")}</section>
