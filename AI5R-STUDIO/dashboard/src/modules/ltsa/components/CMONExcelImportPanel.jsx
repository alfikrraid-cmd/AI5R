...
import {
  getPumps,
  parseConditionMonitoringExcel,
  downloadConditionMonitoringImportTemplate,
} from "../../../api/ai5rClient";
import { mapPumpRecord } from "../utils/pumpMapping";
import { buildImportPreview } from "../utils/conditionMonitoringExcelImport";
...
// MWO-LTSA-CMON-EXCEL-IMPORT-001 -- MANDATORY FLOW: Excel -> parse ->
// map -> Bulk Editor -> validate -> human review/correction -> Confirm
// Create -> the existing atomic bulk endpoint. This component does ONLY
// the first two steps (upload/parse/preview) -- it never calls
// createAdHocConditionMonitoringReadingsBulk itself, has no Confirm-
// style action of its own, and the ONLY way rows leave this component
// is onReviewInBulkEditor(rows), which hands them to the SAME
// BulkCMONReadingEditor Phase 4F.3 built (never a second editor).
// Structural workbook problems (missing Pump Tag column, >1000 rows)
// stop here entirely -- "Review in Bulk Editor" simply does not render
// for them. Row-level problems are attached to individual rows by
// buildImportPreview() and DO proceed into the Bulk Editor, so the user
// can correct them there instead of losing the row.
export default function CMONExcelImportPanel({ onClose, onReviewInBulkEditor }) {
  ...
  function acceptFile(candidate) {
    ...
    // .xlsx only; .xls and .csv are both deliberately rejected.
  ...
  async function handleParse() {
    ...
    try {
      const [parseResult, pumpRecords] = await Promise.all([
        parseConditionMonitoringExcel(file),
  ...
  async function handleDownloadTemplate() {
    ...
    try {
      const blob = await downloadConditionMonitoringImportTemplate();
      const url = URL.createObjectURL(blob);
      const link = document.createElement("a");
      link.href = url;
      link.download = "condition_monitoring_import_template.xlsx";
  ...
  return (
    <div data-testid="cmon-excel-import-panel">
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: spacing.md }}>
        <div>
          <h1 style={{ margin: 0, color: colors.text }}>Import Excel</h1>
          <p style={{ margin: 0, color: colors.textMuted, fontSize: 12 }}>
            Parsed rows are reviewed and corrected in the Bulk Condition Monitoring Reading editor -- nothing is
            created here.
          </p>
        </div>
      ...
      <div
        data-testid="cmon-excel-dropzone"
      ...
      >
        <p style={{ margin: 0, color: colors.textMuted }}>
          Drag &amp; drop the Condition Monitoring .xlsx file here, or click to choose a file.
        </p>
        <input
          ref={inputRef}
          type="file"
          accept=".xlsx"
          aria-label="Upload Condition Monitoring Excel file"
      ...
      {preview ? (
        <div data-testid="cmon-excel-import-preview" style={{ marginTop: spacing.md }}>
